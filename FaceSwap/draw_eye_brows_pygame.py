...
class Visualizer:
    ...
    def draw_face_mesh(self, frame, roi, landmarks, head_pose):
        ...
        if self.shapes2D is not None:
            for shape2D in self.shapes2D:
                ...
                modelParams = self.projectionModel.getInitialParameters(self.mean3DShape[:, self.idxs3D], shape2D[:, self.idxs2D])

                #3D model parameter optimization
                modelParams = NonLinearLeastSquares.GaussNewton(modelParams, self.projectionModel.residual, self.projectionModel.jacobian, ([self.mean3DShape[:, self.idxs3D], self.blendshapes[:, :, self.idxs3D]], shape2D[:, self.idxs2D]), verbose=0)
                ...
                renderedImg = ImageProcessing.colorTransfer(frame, renderedImg, mask)
                ...
                if self.drawOverlay:
                    ...
                    drawProjectedShape(frame, [self.mean3DShape, self.blendshapes], 
                                    self.projectionModel, 
                                    self.mesh, 
                                    modelParams, 
                                    self.lockedTranslation )
    ...
    def should_stop_display(self):
        ...
        if key == ord('r'):
            if self.writer is None:
                ...
                self.writer = cv2.VideoWriter("../out.avi", cv2.VideoWriter.fourcc(*'MJPG'), 25, (frame.shape[1], frame.shape[0]))
    ...
    def process(self, input_stream, output_stream):
        ...
        while input_stream.isOpened():
            ...
            # print ("##"*100)
            # to be removed
            self.shapes2D = utils.getFaceKeypoints(frame, self.detector, self.predictor, self.maxImageSizeForDetection)
    ...
    def run(self, args):
        ...
        cameraImg = input_stream.read()[1]
        self.textureCoords = utils.getFaceTextureCoords(self.textureImg, self.mean3DShape, self.blendshapes, self.idxs2D, self.idxs3D, self.detector, self.predictor)
        self.renderer = FaceRendering.FaceRenderer(cameraImg, self.textureImg, self.textureCoords, self.mesh)
